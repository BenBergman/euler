...
fn find_largest_product_in_matrix() -> u64 {
    let m = vec![
    ...
    for x in 0..m.len() {
        for y in 0..m[0].len() {
            if x >= 3 {
                products.push(m[x][y] * m[x-1][y] * m[x-2][y] * m[x-3][y]);
                println!("{} * {} * {} * {} = {}", m[x][y], m[x-1][y], m[x-2][y], m[x-3][y], products.last().unwrap());
            }
            if y >= 3 {
                products.push(m[x][y] * m[x][y-1] * m[x][y-2] * m[x][y-3]);
            }
            if x >= 3 && y >= 3 {
                products.push(m[x][y] * m[x-1][y-1] * m[x-2][y-2] * m[x-3][y-3]);
            }
            if x <= 16 && y >= 3 {
                products.push(m[x][y] * m[x+1][y-1] * m[x+2][y-2] * m[x+3][y-3]);
            }
...
#[test]
fn matches_solution() {
    assert_eq!(find_largest_product_in_matrix(), 70_600_674);
}
